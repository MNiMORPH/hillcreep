...
def test_velocity_field_integrates_to_the_flux():
    ...
    # negligible, and compare with the closed-form flux at nodes.
    zeta = np.linspace(0.0, 40.0 * h.H_star, 200001)
    ...
    q_closed = -h.diffusivity * np.gradient(h.z, h.dx, edge_order=2)
...
def test_face_form_flux_matches_the_constant_diffusivity_stencil():
    """With K and H_star uniform, face form == the familiar second difference.
    ...
    h.z = np.random.default_rng(0).normal(size=h.x.size)

    face_form = -np.diff(h.flux()) / h.dx
    stencil = (h.diffusivity
